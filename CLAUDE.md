...
10. If work from earlier in the conversation no longer exists on disk (reverted, discarded, working tree back to an older
    commit) and the new request doesn't explicitly say to recover or continue it, treat it as a fresh task on the current
...
- Read `.env`, `.env.*`, or anything under `secrets/` (also enforced in `.claude/settings.json` — if this file changes, update permissions there too)
- // ...
